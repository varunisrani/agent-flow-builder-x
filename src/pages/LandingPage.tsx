      ...
      <header className="h-16 flex-shrink-0 flex items-center justify-between px-4 md:px-8 relative z-50 bg-gradient-to-tr from-zinc-300/5 via-purple-400/10 to-transparent backdrop-blur-sm border-b border-white/5">
        ...
        <nav className="hidden md:flex items-center space-x-1">
          {navigationItems.map((item) => (
            <button
              ...
              className="relative px-4 py-2 text-sm font-medium text-gray-300 hover:text-gray-100 rounded-lg transition-all duration-300 group"
            >
              <span className="relative z-10">{item.name}</span>
              <div className="absolute inset-0 bg-gradient-to-tr from-zinc-300/10 via-purple-400/10 to-transparent rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
          ...
          {user ? (
            ...
            <>
              <Button 
                ...
                className="relative text-gray-300 hover:text-gray-100 hover:bg-gradient-to-tr hover:from-zinc-300/10 hover:via-purple-400/10 hover:to-transparent transition-all duration-300 group"
              >
                <span className="relative z-10">Sign In</span>
                <div className="absolute inset-0 border border-purple-400/20 rounded-md opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
              </Button>
              <span className="relative inline-block overflow-hidden rounded-full p-[1px] group">
                ...
                <div className="inline-flex h-full w-full cursor-pointer items-center justify-center rounded-full bg-gray-950 backdrop-blur-3xl">
                  <Button
                    onClick={() => navigate('/auth')}
                    className="inline-flex rounded-full text-center group items-center justify-center bg-gradient-to-tr from-zinc-300/5 via-purple-400/20 to-transparent text-white border-0 hover:bg-gradient-to-tr hover:from-zinc-300/10 hover:via-purple-400/30 hover:to-transparent transition-all duration-300 px-6 py-2 text-sm font-medium hover:scale-105"
                  >
                    Get Started
        ...
        <div className="md:hidden">
          <Button
            ...
            className="p-2 text-gray-300 hover:text-gray-100 hover:bg-gradient-to-tr hover:from-zinc-300/10 hover:via-purple-400/10 hover:to-transparent transition-all duration-300"
          >
            {isMobileMenuOpen ? (
              <X className="w-6 h-6" />
            ) : (
        ...
        {/* Mobile Menu Overlay */}
        {isMobileMenuOpen && (
          <div className="absolute top-16 left-0 right-0 bg-gradient-to-tr from-zinc-300/10 via-purple-400/20 to-transparent backdrop-blur-xl border-b border-white/5 md:hidden">
            <nav className="flex flex-col p-4 space-y-2">
              {navigationItems.map((item) => (
                <button
                  key={item.name}
                  onClick={() => scrollToSection(item.href)}
                  className="text-left px-4 py-3 text-sm font-medium text-gray-300 hover:text-gray-100 rounded-lg hover:bg-gradient-to-tr hover:from-zinc-300/10 hover:via-purple-400/10 hover:to-transparent transition-all duration-300"
                >
                  {item.name}
                </button>
              ))}
                ...
                {user ? (
                  ...
                  <>
                    <Button 
                      variant="ghost" 
                      onClick={() => navigate('/auth')}
                      className="justify-start text-gray-300 hover:text-gray-100 hover:bg-gradient-to-tr hover:from-zinc-300/10 hover:via-purple-400/10 hover:to-transparent transition-all duration-300"
                    >
                      Sign In
                    </Button>
                    <span className="relative inline-block overflow-hidden rounded-full p-[1px] w-full">
                      ...
                      <div className="inline-flex h-full w-full cursor-pointer items-center justify-center rounded-full bg-gray-950 backdrop-blur-3xl">
                        <Button
                          onClick={() => navigate('/auth')}
                          className="inline-flex rounded-full text-center group items-center justify-center bg-gradient-to-tr from-zinc-300/5 via-purple-400/20 to-transparent text-white border-0 hover:bg-gradient-to-tr hover:from-zinc-300/10 hover:via-purple-400/30 hover:to-transparent transition-all duration-300 px-6 py-3 text-sm font-medium w-full"
                        >
                          Get Started
                          <Sparkles className="w-4 h-4 ml-2 group-hover:rotate-12 transition-transform duration-300" />
                        </Button>
