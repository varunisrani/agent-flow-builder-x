...
app.post('/api/execute', async (req, res) => {
  ...
  try {
    ...
    for (const [filename, content] of Object.entries(files)) {
      // Validate the Python code before writing to prevent syntax errors
      if (filename === 'agent.py') {
        try {
          // Create a temporary Python syntax validation script
...
`;
          ...
          if (syntaxCheck.exitCode !== 0) {
            console.error('❌ Python syntax validation failed:', syntaxCheck.stdout);
            return res.status(400).json({ 
              error: `Python syntax error in ${filename}: ${syntaxCheck.stdout}`,
              executionTime: Date.now() - startTime
            });
          }
          console.log('✅ Python syntax validation passed for agent.py');
        } catch (validationError) {
          console.warn('⚠️ Could not validate Python syntax, proceeding anyway:', validationError.message);
        }
      }
    ...
    console.log('📝 Creating enhanced __init__.py file...');
    const initContent = `"""
Agent Package
...
`;
    ...
    console.log('✅ Created enhanced __init__.py file');
    ...
    try {
      ...
      try {
        ...
        console.log('✅ Successfully created venv with Python 3');
      } catch (fallbackError) {
        console.log('⚠️ Python 3 not available, using system Python');
        venvResult = await sbx.commands.run('python -m venv workspace/venv');
      }
    }
    ...
    // Try multiple installation approaches for better reliability
    let pipResult;
    try {
      // First try with pip upgrade and google-adk
      pipResult = await sbx.commands.run('source workspace/venv/bin/activate && pip install --upgrade pip && pip install google-adk -v', { timeoutMs: 180000 });
    } catch (error) {
      console.log('⚠️ Primary installation failed, trying alternative approach...');
      try {
        // Fallback: Install with no-cache-dir and force-reinstall
        pipResult = await sbx.commands.run('source workspace/venv/bin/activate && pip install google-adk --no-cache-dir --force-reinstall', { timeoutMs: 180000 });
      } catch (fallbackError) {
        console.log('⚠️ Both installation attempts failed, continuing without google-adk');
        pipResult = { exitCode: 1, stdout: 'Installation failed', stderr: fallbackError.message };
      }
    }
    ...
    console.log('✅ ADK config file created with multiple API keys');
...
def wait_for_port(port, max_attempts=45, delay=2):
    ...
    for attempt in range(max_attempts):
        ...
        print(f"⏳ Attempt {attempt + 1}/{max_attempts} (elapsed: {elapsed}s)")
        ...
        if not adk_running and attempt > 5:  # Give it some time to start
            print("❌ ADK web process is not running")
            check_logs()
            return False
        elif adk_running:
            print(f"✓ ADK process is running (PID: {adk_running})")
    ...
    print(f"❌ Timed out waiting for port {port} after {max_attempts * delay} seconds")
    check_logs()
    return False
...
echo "🚀 Starting ADK agent startup process..."
...
if [ -f agent_package/agent.py ]; then
    python3 -m py_compile agent_package/agent.py || {
        echo "❌ Python syntax error in agent.py"
        exit 1
    }
    echo "✅ Agent code validation passed"
else
    echo "❌ agent_package/agent.py not found"
    exit 1
fi
...
if ! command -v adk &> /dev/null; then
    echo "❌ ADK command not found. Attempting to install..."
...
python3 -c "import google.adk.agents; print('✅ Google ADK is properly installed')" || {
    echo "❌ Google ADK import test failed"
    exit 1
}
...
if netstat -tuln | grep -q ":8000 "; then
    echo "⚠️ Port 8000 is already in use, forcefully freeing it..."
    lsof -ti:8000 | xargs kill -9 2>/dev/null || true
    sleep 3
    # Double check
    if netstat -tuln | grep -q ":8000 "; then
        echo "❌ Could not free port 8000, trying alternative approach..."
...
echo "  • Host: 0.0.0.0"
echo "  • Port: 8000"
echo "  • App: agent_package"
echo "  • Python Path: \$PYTHONPATH"

# Function to start ADK server
start_adk_server() {
    local attempt=\$1
    echo "🚀 ADK startup attempt \$attempt..."
    ...
    echo "📋 ADK web server started with PID: \$ADK_PID"
    ...
    if ! kill -0 \$ADK_PID 2>/dev/null; then
        echo "❌ ADK web server process died immediately on attempt \$attempt"
        echo "📋 Error logs:"
        tail -n 20 adk_web.log
        return 1
    fi
    
    echo "✅ ADK web server process is running (attempt \$attempt)"
    return 0
}
...
for attempt in \$(seq 1 \$MAX_ATTEMPTS); do
    if start_adk_server \$attempt; then
        ...
        echo "⚠️ Attempt \$attempt failed, cleaning up and retrying..."
        pkill -f "adk web" 2>/dev/null || true
        sleep 5
    else
...
if [ \$exit_code -eq 0 ]; then
    ...
    echo "  • Port: 8000"
    echo "  • App: agent_package"
    echo "  • PID: \$(cat adk_web.pid)"
    
    # Test the server endpoint
    echo "🧪 Testing server endpoint..."
    curl -s -o /dev/null -w "HTTP Status: %{http_code}\\n" http://localhost:8000/ || echo "⚠️ Could not test endpoint"
    
    echo "📝 Recent logs:"
    tail -n 15 adk_web.log
    exit 0
else
    echo "❌ Failed to start ADK web server within timeout"
    echo "📋 Process status:"
    if kill -0 \$ADK_PID 2>/dev/null; then
        echo "  • Process is still running but port not responding"
    else
    ...
    exit 1
      ...
      while (retryCount <= maxRetries) {
        try {
          ...
          retryCount++;
          console.log(`❌ ADK startup attempt ${retryCount} failed:`, startupError.message);
          
          if (retryCount <= maxRetries) {
            console.log(`🔄 Retrying ADK startup (attempt ${retryCount + 1}/${maxRetries + 1})...`);
            ...
            try {
              ...
              if (fallbackCheck.stdout !== "Failed") {
                ...
                console.log('✅ Fallback startup method succeeded');
                break;
              }
            } catch (fallbackError) {
      ...
      try {
        ...
        console.log(`✅ HTTP server response check: ${healthCheck.stdout}`);
      } catch (error) {
      ...
      const finalResponse = {
        output: `🚀 Agent successfully deployed and accessible!\n\n📊 Deployment Details:\n• Agent Package: agent_package\n• Server Status: Running\n• Access URL: ${agentUrl}\n\n🔗 Click "Open Agent UI" to interact with your agent.`,
        error: null,
        executionTime: Date.now() - startTime,
      ...
      console.log(`🚀 Agent successfully deployed!`);
      console.log(`📊 Deployment details:`);
      console.log(`• Package: ${finalResponse.agentPackage}`);
      console.log(`• Status: ${finalResponse.executionDetails.status}`);
      console.log(`• Duration: ${finalResponse.executionDetails.duration} ms`);
      console.log(`• Agent URL: ${finalResponse.executionDetails.serverUrl}`);

      console.log('\n📈 Execution Metadata:');
      console.log(`• Execution Time: ${finalResponse.executionTime}ms`);
      console.log(`• Memory Usage: ${finalResponse.memoryUsage.toFixed(2)}MB`);
      console.log(`• Deployment Status: ${finalResponse.deployment.status}`);
      console.log(`• Access URL: ${finalResponse.deployment.url}`);
...
if (process.env.NODE_ENV !== 'vercel') {
  app.listen(PORT, () => {
    console.log(`🚀 Server is running on http://localhost:${PORT}`);
      ...
      while (retryCount <= maxRetries) {
        try {
          ...
          if (retryCount <= maxRetries) {
            ...
            try {
              // Simple direct startup without the complex script
              const fallbackResult = await sbx.commands.run(
                'cd workspace && source venv/bin/activate && export GOOGLE_API_KEY=' + 
                (process.env.GOOGLE_API_KEY || 'AIzaSyABISxaNzifdIcZUCe408LoKnEz0bia8cI') + 
                ' && timeout 60 adk web --host 0.0.0.0 --port 8000 --app agent_package &', 
                { timeoutMs: 70000 }
      ...
      // Create agent-specific URL with proper query parameters
      const agentUrl = new URL(publicUrl);
      agentUrl.searchParams.set('app', 'agent_package');
      const finalAgentUrl = agentUrl.toString();
      ...
      const response = {
        ...
        executionDetails: {
          ...
          status: 'running',
          duration: Date.now() - startTime,
          serverUrl: finalAgentUrl // Use the agent-specific URL
        },
        // Enhanced fields for the frontend CodeGenerationModal
        openUrl: finalAgentUrl,
        showOpenLink: true,
        linkText: 'Open Agent UI',
        agentPackage: 'agent_package',
        deployment: {
      ...
      console.log(`🚀 Agent successfully deployed!`);
      console.log(`📊 Deployment details:`);
      console.log(`• Package: ${response.agentPackage}`);
      console.log(`• Status: ${response.executionDetails.status}`);
      console.log(`• Duration: ${response.executionDetails.duration} ms`);
      console.log(`• Agent URL: ${response.executionDetails.serverUrl}`);

      console.log('\n📈 Execution Metadata:');
      console.log(`• Execution Time: ${response.executionTime}ms`);
      console.log(`• Memory Usage: ${response.memoryUsage.toFixed(2)}MB`);
      console.log(`• Deployment Status: ${response.deployment.status}`);
      console.log(`• Access URL: ${response.deployment.url}`);
      ...
      try {
        if (sbx) {
          console.log('🧹 Final sandbox cleanup...');
          if (typeof sbx.destroy === 'function') {
            await sbx.destroy();
          } else if (typeof sbx.close === 'function') {
            await sbx.close();
